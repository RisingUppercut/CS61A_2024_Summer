...
def balanced(m):
    ...
    torque_l, torque_r = length(left_arm) * total_mass(end(left_arm)), length(right_arm) * total_mass(end(right_arm))
    return torque_l == torque_r and balanced(end(left_arm)) and balanced(end(right_arm))
...
def max_path_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    sum, max_of_branches = label(t), 0
    if is_leaf(t): 
        return sum
    for branch in branches(t):
        max_of_branches = max(max_of_branches, max_path_sum(branch))
    return sum + max_of_branches
