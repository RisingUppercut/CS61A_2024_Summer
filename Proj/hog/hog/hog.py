...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def stragegy(player_score, opponent_score):
        return n
    return stragegy
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    i = 0
    pre_max_scoring, max_scoring_rolls = 0, 0
    averaged_dice = make_averaged(roll_dice, samples_count)
    while i < 10:
        i += 1
        current_scoring = averaged_dice(i, dice)
        if  current_scoring > pre_max_scoring:
            max_scoring_rolls = i
            pre_max_scoring = current_scoring
