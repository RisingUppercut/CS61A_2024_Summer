...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def func(paragraph):
        words_list = split(remove_punctuation(paragraph))
        for w in words_list:
            w = lower(w)
            if w in subject:
                return True
...
def accuracy(typed, source):
    ...
    if len_typed == 0 and len_source == 0:
        ...
        return 0.0
    i, count = 0, 0
    while i < len_source and i < len_typed:
...
def wpm(typed, elapsed):
    ...
    minutes = elapsed / 60
    return characters_length / 5 / minutes
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    smallest_diff_word = min(word_list, key = lambda s: diff_function(typed_word, s, limit))
    if diff_function(typed_word, smallest_diff_word, limit) > limit or typed_word in word_list:
        return typed_word
    else:
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if source[i] == typed[i]:
            ...
            break
    user_progress = corret_words / len(source)
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    words, times = get_all_words(match_object), get_all_times(match_object)
    "*** YOUR CODE HERE ***"
    L = []
    ...
    for i in word_indices:
        ...
        L[fastest_player_index].append(words[i])
    return L
